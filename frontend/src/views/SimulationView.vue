...
const handleNextStep = (params = {}) => {
  ...
  // 사용자 정의 라운드 수가 있으면 query 파라미터로 전달
  if (params.maxRounds) {
...
/**
 * 실행 중인 시뮬레이션 확인 및 종료
 * 사용자가 Step 3에서 Step 2로 돌아올 때, 시뮬레이션 종료 의도로 간주
 */
const checkAndStopRunningSimulation = async () => {
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      addLog('시뮬레이션 환경이 실행 중인 것을 감지, 종료 중...')
      ...
      // 환경 미실행, 하지만 프로세스가 남아 있을 수 있으므로 시뮬레이션 상태 확인
      const simRes = await getSimulation(currentSimulationId.value)
      if (simRes.success && simRes.data?.status === 'running') {
        addLog('시뮬레이션 상태가 실행 중인 것을 감지, 중지 중...')
        await forceStopSimulation()
      }
    }
  } catch (err) {
    // 환경 상태 확인 실패는 후속 프로세스에 영향 없음
...
onMounted(async () => {
  ...
  // 실행 중인 시뮬레이션 확인 및 종료 (사용자가 Step 3에서 복귀 시)
