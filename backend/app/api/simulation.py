...
def optimize_interview_prompt(prompt: str) -> str:
    """
    Interview 질문 최적화, Agent 도구 호출을 방지하기 위한 접두사 추가
...
def get_graph_entities(graph_id: str):
    """
    그래프의 모든 엔티티 조회 (필터링 완료)

    사전 정의된 엔티티 타입에 부합하는 노드만 반환합니다 (Labels가 Entity만이 아닌 노드)

    Query 파라미터:
        entity_types: 쉼표로 구분된 엔티티 타입 목록 (선택, 추가 필터링용)
...
def create_simulation():
    ...
    참고: max_rounds 등의 파라미터는 LLM이 지능적으로 생성하므로 수동 설정이 불필요합니다

    요청 (JSON):
        {
            "project_id": "proj_xxxx",      // 필수
            "graph_id": "mirofish_xxxx",    // 선택, 미제공 시 project에서 조회
    ...
    try:
        ...
        if not graph_id:
            return jsonify({
                "success": False,
                "error": "프로젝트에 그래프가 아직 구축되지 않았습니다. 먼저 /api/graph/build를 호출해주세요"
            }), 400
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    참고: 실행 스크립트(run_*.py)는 backend/scripts/ 디렉토리에 유지되며, 시뮬레이션 디렉토리에 복사하지 않습니다
    ...
    try:
        ...
        # config_generated=True이고 파일이 존재하면 준비 완료로 판단
        # 다음 상태는 모두 준비 작업이 완료되었음을 의미:
        # - ready: 준비 완료, 실행 가능
        # - preparing: config_generated=True이면 이미 완료
        # - running: 실행 중, 준비는 이미 완료
        # - completed: 실행 완료, 준비는 이미 완료
        # - stopped: 중지됨, 준비는 이미 완료
        # - failed: 실행 실패 (하지만 준비는 완료)
...
def prepare_simulation():
    ...
    시간이 소요되는 작업이므로, 인터페이스는 즉시 task_id를 반환합니다.
    GET /api/simulation/prepare/status 로 진행 상황을 조회하세요

    특징:
    - 완료된 준비 작업을 자동 감지하여 중복 생성 방지
    - 이미 준비 완료된 경우 기존 결과를 직접 반환
    ...
    4. LLM으로 시뮬레이션 설정 지능 생성 (재시도 메커니즘 포함)
    5. 설정 파일 및 프리셋 스크립트 저장

    요청 (JSON):
        {
            "simulation_id": "sim_xxxx",                   // 필수, 시뮬레이션 ID
            "entity_types": ["Student", "PublicFigure"],  // 선택, 엔티티 타입 지정
            "use_llm_for_profiles": true,                 // 선택, LLM으로 프로필 생성 여부
            "parallel_profile_count": 5,                  // 선택, 병렬 프로필 생성 수, 기본값 5
    ...
    try:
            ...
            if is_prepared:
                ...
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "ready",
                        "message": "완료된 준비 작업이 있어 중복 생성이 불필요합니다",
                        "already_prepared": True,
                        "prepare_info": prepare_info
                    }
                })
            else:
                logger.info(f"시뮬레이션 {simulation_id} 준비 미완료, 준비 태스크를 시작합니다")
        ...
        # ========== 동기적 엔티티 수량 조회 (백그라운드 태스크 시작 전) ==========
        # 프론트엔드가 prepare 호출 후 즉시 예상 Agent 총 수를 얻을 수 있도록 함
        try:
            logger.info(f"동기적 엔티티 수량 조회: graph_id={state.graph_id}")
            reader = ZepEntityReader()
            # 빠른 엔티티 읽기 (엣지 정보 불필요, 수량만 통계)
            filtered_preview = reader.filter_defined_entities(
                graph_id=state.graph_id,
                defined_entity_types=entity_types_list,
                enrich_with_edges=False  # 엣지 정보 미조회, 속도 향상
            )
            # 엔티티 수량을 상태에 저장 (프론트엔드가 즉시 조회 가능)
            state.entities_count = filtered_preview.filtered_count
            state.entity_types = list(filtered_preview.entity_types)
            logger.info(f"예상 엔티티 수량: {filtered_preview.filtered_count}, 타입: {filtered_preview.entity_types}")
        except Exception as e:
            logger.warning(f"동기적 엔티티 수량 조회 실패 (백그라운드 태스크에서 재시도): {e}")
            # 실패해도 후속 흐름에 영향 없음, 백그라운드 태스크에서 다시 조회
        ...
        # 시뮬레이션 상태 업데이트 (미리 조회한 엔티티 수량 포함)
        ...
        return jsonify({
            ...
            "data": {
                ...
                "task_id": task_id,
                "status": "preparing",
                "message": "준비 태스크가 시작되었습니다. /api/simulation/prepare/status 로 진행 상황을 조회하세요",
                "already_prepared": False,
                "expected_entities_count": state.entities_count,  # 예상 Agent 총 수
                "entity_types": state.entity_types  # 엔티티 타입 목록
            }
        })
...
def get_prepare_status():
    ...
    2. simulation_id로 완료된 준비 작업이 있는지 확인
    ...
    try:
        ...
        if not task:
            # 태스크가 존재하지 않지만 simulation_id가 있으면 준비 완료 여부 확인
            if simulation_id:
                is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
                if is_prepared:
                    return jsonify({
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    """
    시뮬레이션에 대응하는 최신 report_id 조회

    reports 디렉토리를 순회하여 simulation_id가 일치하는 report를 찾고,
    여러 개인 경우 최신 것을 반환 (created_at 기준 정렬)
    ...
    # __file__은 app/api/simulation.py이므로, 두 단계 위로 올라가 backend/에 도달
    ...
    try:
        ...
        logger.warning(f"시뮬레이션 {simulation_id}의 report 조회 실패: {e}")
        return None
...
def get_simulation_history():
    """
    시뮬레이션 이력 목록 조회 (프로젝트 상세 정보 포함)

    홈페이지 이력 프로젝트 표시용으로, 프로젝트 이름, 설명 등 풍부한 정보가 포함된 시뮬레이션 목록을 반환
    ...
    try:
        ...
        # 시뮬레이션 데이터 보강, Simulation 파일에서만 읽기
        enriched_simulations = []
        for sim in simulations:
            sim_dict = sim.to_dict()

            # 시뮬레이션 설정 정보 조회 (simulation_config.json에서 simulation_requirement 읽기)
            ...
            # 실행 상태 조회 (run_state.json에서 사용자가 설정한 실제 라운드 수 읽기)
            ...
            # 연관된 report_id 조회 (해당 시뮬레이션의 최신 report 검색)
        ...
        logger.error(f"시뮬레이션 이력 조회 실패: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def get_simulation_profiles(simulation_id: str):
    ...
    Query 파라미터:
        platform: 플랫폼 타입 (reddit/twitter, 기본값 reddit)
    """
    try:
...
def get_simulation_profiles_realtime(simulation_id: str):
    """
    시뮬레이션의 Agent Profile 실시간 조회 (생성 과정 중 실시간 진행 확인용)

    /profiles 인터페이스와의 차이점:
    - 파일을 직접 읽으며 SimulationManager를 경유하지 않음
    - 생성 과정 중 실시간 확인에 적합
    ...
    try:
        ...
        if file_exists:
            ...
            try:
                ...
                logger.warning(f"profiles 파일 읽기 실패 (쓰기 진행 중일 수 있음): {e}")
                profiles = []
...
def get_simulation_config_realtime(simulation_id: str):
    """
    시뮬레이션 설정 실시간 조회 (생성 과정 중 실시간 진행 확인용)

    /config 인터페이스와의 차이점:
    - 파일을 직접 읽으며 SimulationManager를 경유하지 않음
    - 생성 과정 중 실시간 확인에 적합
    - 추가 메타데이터 반환 (파일 수정 시간, 생성 중 여부 등)
    - 설정이 아직 완료되지 않아도 부분 정보를 반환 가능
    ...
    try:
        ...
        if file_exists:
            ...
            try:
                ...
                logger.warning(f"config 파일 읽기 실패 (쓰기 진행 중일 수 있음): {e}")
                config = None
...
def start_simulation():
    ...
    요청 (JSON):
        {
            ...
            "max_rounds": 100,                     // 선택: 최대 시뮬레이션 라운드 수, 긴 시뮬레이션 절단용
            "enable_graph_memory_update": false,   // 선택: Agent 활동을 Zep 그래프 메모리에 동적 업데이트할지 여부
            "force": false                         // 선택: 강제 재시작 (실행 중인 시뮬레이션을 중지하고 로그 정리)
        }

    force 파라미터에 대해:
        - 활성화 시, 시뮬레이션이 실행 중이거나 완료된 경우 먼저 중지하고 실행 로그를 정리
        - 정리 대상: run_state.json, actions.jsonl, simulation.log 등
        - 설정 파일(simulation_config.json)과 profile 파일은 정리하지 않음
        - 시뮬레이션을 다시 실행해야 하는 시나리오에 적합

    enable_graph_memory_update에 대해:
        - 활성화 시, 시뮬레이션 중 모든 Agent의 활동(게시, 댓글, 좋아요 등)이 실시간으로 Zep 그래프에 업데이트
        - 그래프가 시뮬레이션 과정을 "기억"하여 후속 분석이나 AI 대화에 활용 가능
        - 시뮬레이션과 연관된 프로젝트에 유효한 graph_id가 필요
        - 배치 업데이트 메커니즘으로 API 호출 횟수 절감
    ...
    try:
        ...
        if platform not in ['twitter', 'reddit', 'parallel']:
            return jsonify({
                "success": False,
                "error": f"유효하지 않은 플랫폼 타입: {platform}, 가능한 값: twitter/reddit/parallel"
            }), 400
        ...
        # 상태 지능 처리: 준비 작업이 완료되었으면 재시작 허용
        if state.status != SimulationStatus.READY:
            # 준비 작업 완료 여부 확인
            is_prepared, prepare_info = _check_simulation_prepared(simulation_id)

            if is_prepared:
                # 준비 작업 완료, 실행 중인 프로세스가 있는지 확인
                if state.status == SimulationStatus.RUNNING:
                    ...
                    if run_state and run_state.runner_status.value == "running":
                        ...
                        if force:
                            ...
                            return jsonify({
                                "success": False,
                                "error": f"시뮬레이션이 실행 중입니다. 먼저 /stop 인터페이스를 호출하거나 force=true를 사용하여 강제 재시작하세요"
                            }), 400
        ...
        # 그래프 ID 조회 (그래프 메모리 업데이트용)
        graph_id = None
        if enable_graph_memory_update:
            # 시뮬레이션 상태 또는 프로젝트에서 graph_id 조회
            graph_id = state.graph_id
            if not graph_id:
                # 프로젝트에서 조회 시도
                project = ProjectManager.get_project(state.project_id)
            ...
            if not graph_id:
                return jsonify({
                    "success": False,
                    "error": "그래프 메모리 업데이트를 활성화하려면 유효한 graph_id가 필요합니다. 프로젝트에 그래프가 구축되었는지 확인해주세요"
                }), 400
...
def get_run_status_detail(simulation_id: str):
    ...
    try:
        ...
        # 현재 라운드의 동작 조회 (recent_actions는 최신 라운드만 표시)
        ...
        # recent_actions는 현재 최신 라운드의 양 플랫폼 내용만 표시
...
def get_simulation_posts(simulation_id: str):
    ...
    Query 파라미터:
        platform: 플랫폼 타입 (twitter/reddit)
        limit: 반환 수량 (기본값 50)
        offset: 오프셋

    게시물 목록을 반환 (SQLite 데이터베이스에서 읽기)
    """
    try:
        platform = request.args.get('platform', 'reddit')
...
def interview_agent():
    ...
    참고: 이 기능은 시뮬레이션 환경이 실행 상태여야 합니다 (시뮬레이션 사이클 완료 후 명령 대기 모드 진입)

    요청 (JSON):
        {
            "simulation_id": "sim_xxxx",       // 필수, 시뮬레이션 ID
            "agent_id": 0,                     // 필수, Agent ID
            "prompt": "이 일에 대해 어떻게 생각하시나요?",  // 필수, 인터뷰 질문
            "platform": "twitter",             // 선택, 플랫폼 지정 (twitter/reddit)
                                               // 미지정 시: 듀얼 플랫폼 시뮬레이션에서 양 플랫폼 동시 인터뷰
    ...
    try:
        ...
        if not SimulationRunner.check_env_alive(simulation_id):
            return jsonify({
                "success": False,
                "error": "시뮬레이션 환경이 실행 중이 아니거나 종료되었습니다. 시뮬레이션이 완료되어 명령 대기 모드에 진입했는지 확인해주세요."
            }), 400

        # prompt 최적화, Agent 도구 호출 방지를 위한 접두사 추가
...
def interview_agents_batch():
    ...
    요청 (JSON):
        {
            ...
            "interviews": [                    // 필수, 인터뷰 목록
                {
                    "agent_id": 0,
                    "prompt": "A에 대해 어떻게 생각하시나요?",
                    "platform": "twitter"      // 선택, 해당 Agent의 인터뷰 플랫폼 지정
                },
                {
                    "agent_id": 1,
                    "prompt": "B에 대해 어떻게 생각하시나요?"  // platform 미지정 시 기본값 사용
                }
            ],
            "platform": "reddit",              // 선택, 기본 플랫폼 (각 항목의 platform으로 재정의)
                                               // 미지정 시: 듀얼 플랫폼 시뮬레이션에서 각 Agent를 양 플랫폼 동시 인터뷰
    ...
    try:
        ...
        # 각 인터뷰 항목의 prompt 최적화, Agent 도구 호출 방지를 위한 접두사 추가
...
def interview_all_agents():
    ...
    요청 (JSON):
        {
            "simulation_id": "sim_xxxx",            // 필수, 시뮬레이션 ID
            "prompt": "이 일 전체에 대해 어떻게 생각하시나요?",  // 필수, 인터뷰 질문 (모든 Agent가 동일 질문 사용)
...
def get_interview_history():
    ...
    시뮬레이션 데이터베이스에서 모든 Interview 기록을 읽습니다

    요청 (JSON):
        {
            "simulation_id": "sim_xxxx",  // 필수, 시뮬레이션 ID
            "platform": "reddit",          // 선택, 플랫폼 타입 (reddit/twitter)
    ...
    try:
        ...
        simulation_id = data.get('simulation_id')
        platform = data.get('platform')  # 미지정 시 양 플랫폼 이력 반환
        agent_id = data.get('agent_id')
...
def get_env_status():
    ...
    시뮬레이션 환경이 활성화되어 있는지 확인 (Interview 명령을 수신할 수 있는지)
...
def close_simulation_env():
    ...
    시뮬레이션에 환경 종료 명령을 전송하여 명령 대기 모드를 우아하게 종료합니다.

    참고: /stop 인터페이스와 다릅니다. /stop은 프로세스를 강제 종료하고,
    이 인터페이스는 시뮬레이션이 환경을 우아하게 종료하고 퇴장하게 합니다.
