    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 0, 'completed': phase > 0 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            새 simulation 인스턴스 생성, 시뮬레이션 세계 파라미터 템플릿 로드
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            컨텍스트를 결합하여 지식 그래프에서 엔티티와 관계를 자동으로 정리하고, 시뮬레이션 개체를 초기화하며, 현실 시드를 기반으로 고유한 행동과 기억을 부여합니다
          </p>
          ...
          <div v-if="profiles.length > 0" class="stats-grid">
            ...
            <div class="stat-card">
              ...
              <span class="stat-label">현실 시드 관련 토픽 수</span>
            </div>
          </div>
            ...
            <div class="profiles-list">
              ...
              >
                ...
                <div class="profile-meta">
                  <span class="profile-profession">{{ profile.profession || '미확인 직업' }}</span>
                </div>
      ...
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            LLM이 시뮬레이션 요구사항과 현실 시드를 기반으로, 세계 시간 흐름 속도, 추천 알고리즘, 각 개체의 활동 시간대, 발언 빈도, 이벤트 트리거 등의 파라미터를 지능적으로 설정합니다
          </p>
              ...
              <div class="platforms-grid">
                <div v-if="simulationConfig.twitter_config" class="platform-card">
                  ...
                  <div class="platform-params">
                    <div class="param-row">
                      <span class="param-label">시효성 가중치</span>
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        ...
        <div class="card-content">
          ...
          <!-- 시뮬레이션 라운드 설정 - 설정 생성 완료 후 라운드 수 계산된 후에만 표시 -->
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                <span class="section-title">시뮬레이션 라운드 설정</span>
                <span class="section-desc">MiroFish가 자동으로 현실 시뮬레이션을 계획합니다 <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span> 시간, 각 라운드는 현실 <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span> 분의 시간 흐름을 나타냅니다</span>
              </div>
              ...
              <div v-if="useCustomRounds" class="rounds-content custom" key="custom">
                <div class="slider-display">
                  ...
                  <div class="slider-meta-info">
                    <span>Agent 규모 100 기준: 예상 소요시간 약 {{ Math.round(customMaxRounds * 0.6) }} 분</span>
                  </div>
                </div>
                  ...
                  <div class="auto-content">
                    <div class="auto-meta-row">
                      <span class="duration-badge">
                        ...
                        Agent 규모 100 기준: 예상 소요시간 약 {{ Math.round(autoGeneratedRounds * 0.6) }} 분
                      </span>
                    </div>
                    <div class="auto-desc">
                      <p class="highlight-tip" @click="useCustomRounds = true">첫 실행 시, ‘사용자 정의 모드’로 전환하여 라운드 수를 줄이면 빠른 미리보기와 오류 위험 감소에 도움됩니다 ➝</p>
                    </div>
                  </div>
                </div>
              </div>
...
// 로그 중복 제거: 마지막 출력의 주요 정보 기록
let lastLoggedMessage = ''
let lastLoggedProfileCount = 0
let lastLoggedConfigStage = ''

// 시뮬레이션 라운드 설정
const useCustomRounds = ref(false) // 기본값: 자동 설정 라운드 사용
...
// 설정에서 자동 생성 라운드 수 계산 (하드코딩된 기본값 사용하지 않음)
const autoGeneratedRounds = computed(() => {
  ...
  // 최대 라운드가 40 미만이 되지 않도록 보장 (권장값), 슬라이더 범위 이상 방지
...
const handleStartSimulation = () => {
  ...
  if (useCustomRounds.value) {
    ...
    addLog(`시뮬레이션 시작, 사용자 정의 라운드: ${customMaxRounds.value} 라운드`)
  } else {
    // 사용자가 자동 생성 라운드 유지 선택, max_rounds 파라미터 전달하지 않음
...
const startPrepareSimulation = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // 예상 Agent 총수 즉시 설정 (prepare 인터페이스 반환값에서 획득)
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (currentCount > 0 && currentCount !== lastLoggedProfileCount) {
        ...
        addLog(`→ Agent 페르소나 ${currentCount}/${total}: ${profileName} (${latestProfile?.profession || '미확인 직업'})`)
