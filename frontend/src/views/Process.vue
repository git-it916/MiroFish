            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  문서 업로드 후, LLM이 문서 내용을 분석하여 여론 시뮬레이션에 적합한 온톨로지 구조를 자동 생성합니다 (엔티티 유형 + 관계 유형)
                </div>
              </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">인터페이스 설명</div>
                <div class="detail-content">
                  생성된 온톨로지를 기반으로 문서를 분할한 후 Zep API를 호출하여 지식 그래프를 구축하고, 엔티티와 관계를 추출합니다
                </div>
              </div>
...
// 새 프로젝트 처리 - ontology/generate API 호출
const handleNewProject = async () => {
  const pending = getPendingUpload()
  
  if (!pending.isPending || pending.files.length === 0) {
    error.value = '업로드 대기 파일이 없습니다. 홈페이지로 돌아가서 다시 시도하세요'
    loading.value = false
    return
  }
...
const startBuildGraph = async () => {
  try {
    ...
    if (response.success) {
      buildProgress.value.message = '그래프 구축 작업이 시작되었습니다...'
...
const fetchGraphData = async () => {
  try {
    // graph_id를 얻기 위해 먼저 프로젝트 정보 가져오기
