"""
그래프 관련 API 라우트
프로젝트 컨텍스트 메커니즘을 사용하며, 서버 측에서 상태를 영속화
"""
...
def generate_ontology():
    """
    인터페이스1: 파일 업로드 후 분석하여 온톨로지 정의 생성
    ...
    try:
        ...
        if not document_texts:
            ...
            return jsonify({
                "success": False,
                "error": "문서 처리에 성공한 항목이 없습니다. 파일 형식을 확인해주세요"
            }), 400
...
def build_graph():
    """
    인터페이스2: project_id 기반으로 그래프 구축

    요청 (JSON):
        {
            "project_id": "proj_xxxx",  // 필수, 인터페이스1에서 획득
    ...
    try:
        ...
        if project.status == ProjectStatus.CREATED:
            return jsonify({
                "success": False,
                "error": "프로젝트에 온톨로지가 아직 생성되지 않았습니다. 먼저 /ontology/generate를 호출해주세요"
            }), 400

        if project.status == ProjectStatus.GRAPH_BUILDING and not force:
            return jsonify({
                "success": False,
                "error": "그래프가 구축 중입니다. 중복 제출하지 마세요. 강제 재구축이 필요하면 force: true를 추가하세요",
        ...
        return jsonify({
            ...
            "data": {
                "project_id": project_id,
                "task_id": task_id,
                "message": "그래프 구축 태스크가 시작되었습니다. /task/{task_id}로 진행 상황을 조회하세요"
            }
        })
