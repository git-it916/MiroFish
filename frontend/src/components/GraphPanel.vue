    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">일부 콘텐츠 처리 중, 나중에 수동으로 그래프를 새로고침하세요</span>
...
const wasSimulating = ref(false) // 이전 시뮬레이션 상태 추적
...
// isSimulating 변화 감시, 시뮬레이션 종료 감지
watch(() => props.isSimulating, (newValue, oldValue) => {
  if (wasSimulating.value && !newValue) {
    // 시뮬레이션 중에서 비시뮬레이션 상태로 전환, 종료 알림 표시
...
const renderGraph = () => {
  ...
  // 엣지 데이터 처리, 동일 노드 쌍 간의 엣지 수량과 인덱스 계산
  const edgePairCount = {}
  const selfLoopEdges = {} // 노드별로 그룹화된 셀프루프 엣지
  const tempEdges = edgesData
    .filter(e => nodeIds.has(e.source_node_uuid) && nodeIds.has(e.target_node_uuid))
  
  // 각 노드 쌍 간의 엣지 수량 통계, 셀프루프 엣지 수집
  tempEdges.forEach(e => {
  ...
  // 각 노드 쌍의 현재 처리된 엣지 번호 기록
  const edgePairIndex = {}
  const processedSelfLoopNodes = new Set() // 처리 완료된 셀프루프 노드
  ...
  tempEdges.forEach(e => {
    ...
    if (isSelfLoop) {
      // 셀프루프 엣지 - 각 노드당 하나의 병합된 셀프루프만 추가
    ...
    // 엣지 방향이 표준화 방향과 일치하는지 판단 (소스UUID < 대상UUID)
    const isReversed = e.source_node_uuid > e.target_node_uuid
    
    // 곡률 계산: 다중 엣지 시 분산, 단일 엣지는 직선
    let curvature = 0
    if (totalCount > 1) {
      // 곡률 균등 분배, 명확한 구분 보장
      ...
      // 엣지 방향이 표준화 방향과 반대인 경우 곡률 반전
      // 이를 통해 모든 엣지가 동일 참조 프레임에서 분포되어 방향 차이로 인한 겹침 방지
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  // Simulation - 엣지 수량에 따라 노드 간격 동적 조정
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      // 노드 쌍 간의 엣지 수량에 따라 거리 동적 조정
      // 기본 거리 150, 엣지 하나 추가 시 50 증가
    ...
    // 중심 방향 인력 추가, 독립 노드 그룹을 중심 영역으로 집결
  ...
  const getLinkPath = (d) => {
    ...
    // 셀프루프 감지
    if (d.isSelfLoop) {
      // 셀프루프: 노드에서 출발하여 원호를 그리고 돌아오는 경로
      const loopRadius = 30
      // 노드 오른쪽에서 출발하여 한 바퀴 돌아옴
    ...
    // 곡선 제어점 계산 - 엣지 수량과 거리에 따라 동적 조정
    const dx = tx - sx, dy = ty - sy
    const dist = Math.sqrt(dx * dx + dy * dy)
    // 연결선 방향에 수직인 오프셋, 거리 비율에 따라 계산하여 곡선 가시성 보장
    // 엣지가 많을수록 오프셋이 거리에서 차지하는 비율 증가
    const pairTotal = d.pairTotal || 1
    const offsetRatio = 0.25 + pairTotal * 0.05 // 기본 25%, 엣지 하나 추가 시 5% 증가
  ...
  const getLinkMidpoint = (d) => {
    ...
    // 2차 베지어 곡선 공식 B(t) = (1-t)²P0 + 2(1-t)tP1 + t²P2, t=0.5
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      ...
      .on('drag', (event, d) => {
        // 실제 드래그 시작 감지 (이동 거리 임계값 초과)
        const dx = event.x - d._dragStartX
        const dy = event.y - d._dragStartY
        const distance = Math.sqrt(dx * dx + dy * dy)
        
        if (!d._isDragging && distance > 3) {
          // 실제 드래그 최초 감지 시 시뮬레이션 재시작
          d._isDragging = true
