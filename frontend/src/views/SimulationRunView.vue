...
// 초기화 시 query 파라미터에서 maxRounds를 직접 가져와 자식 컴포넌트가 즉시 값을 받을 수 있도록 함
...
const handleGoBack = async () => {
  // Step 2로 복귀 전, 실행 중인 시뮬레이션 종료
  addLog('Step 2 복귀 준비, 시뮬레이션 종료 중...')
...
const handleNextStep = () => {
  // Step3Simulation 컴포넌트에서 보고서 생성 및 라우트 이동을 직접 처리
  // 이 메서드는 예비용
...
const loadGraph = async (graphId) => {
  // 시뮬레이션 중 자동 새로고침 시 전체 화면 loading을 표시하지 않아 깜빡임 방지
