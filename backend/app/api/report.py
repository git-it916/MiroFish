...
def generate_report():
    ...
    GET /api/report/generate/status 로 진행 상황을 조회하세요
    ...
    try:
        ...
        return jsonify({
            ...
            "data": {
                ...
                "task_id": task_id,
                "status": "generating",
                "message": "보고서 생성 태스크가 시작되었습니다. /api/report/generate/status 로 진행 상황을 조회하세요",
...
def get_generate_status():
    ...
    요청 (JSON):
        {
            "task_id": "task_xxxx",         // 선택, generate가 반환한 task_id
    ...
    try:
        ...
        # simulation_id가 제공된 경우, 먼저 완료된 보고서가 있는지 확인
        if simulation_id:
            existing_report = ReportManager.get_report_by_simulation(simulation_id)
            if existing_report and existing_report.status == ReportStatus.COMPLETED:
                return jsonify({
...
def get_report_sections(report_id: str):
    ...
    프론트엔드에서 이 인터페이스를 폴링하여 생성된 챕터 내용을 가져올 수 있으며,
    전체 보고서 완료를 기다릴 필요가 없습니다
    ...
    try:
        ...
        logger.error(f"챕터 목록 조회 실패: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def get_single_section(report_id: str, section_index: int):
    ...
    try:
        ...
        if not os.path.exists(section_path):
            return jsonify({
                "success": False,
                "error": f"챕터가 존재하지 않습니다: section_{section_index:02d}.md"
            }), 404
        ...
        logger.error(f"챕터 내용 조회 실패: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def get_agent_log(report_id: str):
    ...
    보고서 생성 과정의 각 단계 동작을 실시간으로 조회합니다:
    - 보고서 시작, 계획 시작/완료
    - 각 챕터의 시작, 도구 호출, LLM 응답, 완료
    - 보고서 완료 또는 실패

    Query 파라미터:
        from_line: 몇 번째 줄부터 읽을지 (선택, 기본값 0, 증분 조회에 사용)
...
def get_console_log(report_id: str):
    ...
    보고서 생성 과정의 콘솔 출력 (INFO, WARNING 등)을 실시간으로 조회합니다.
