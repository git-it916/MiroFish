<template>
  ...
  >
    ...
    <div v-if="projects.length > 0" class="cards-container" :class="{ expanded: isExpanded }" :style="containerStyle">
      ...
      >
        <!-- 카드 헤더: simulation_id 및 기능 가용 상태 -->
        ...
        <!-- 카드 타이틀 (시뮬레이션 요구사항의 처음 20자를 타이틀로 사용) -->
        ...
        <div v-if="selectedProject" class="modal-overlay" @click.self="closeModal">
          <div class="modal-content">
            ...
            <div class="modal-divider">
              ...
              <span class="divider-text">시뮬레이션 리플레이</span>
              <span class="divider-line"></span>
            </div>
            ...
            <div class="modal-playback-hint">
              <span class="hint-text">Step3 "시뮬레이션 시작"과 Step5 "심층 상호작용"은 실행 중에만 시작 가능하며, 기록 리플레이를 지원하지 않습니다</span>
            </div>
          </div>
        </div>
      </Transition>
...
let pendingState = null  // 대기 중인 목표 상태 기록
...
const containerStyle = computed(() => {
  ...
  // 실제 필요 높이 계산: 행 수 * 카드 높이 + (행 수-1) * 간격 + 약간의 하단 간격
...
// 라운드 수 표시 포맷팅 (현재 라운드/총 라운드)
const formatRounds = (simulation) => {
  const current = simulation.current_round || 0
  const total = simulation.total_rounds || 0
  if (total === 0) return '시작 안 됨'
...
const loadHistory = async () => {
  try {
    ...
    console.error('기록 프로젝트 로드 실패:', error)
    projects.value = []
  } finally {
    loading.value = false
  }
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // 대기 중인 목표 상태 업데이트 (애니메이션 중이든 아니든 최신 목표 상태 기록)
        pendingState = shouldExpand
        
        // 이전 디바운스 타이머 제거 (새로운 스크롤 의도가 기존 것을 대체)
        if (expandDebounceTimer) {
          clearTimeout(expandDebounceTimer)
          expandDebounceTimer = null
        }
        
        // 애니메이션 중이면 상태만 기록하고 애니메이션 종료 후 처리
        if (isAnimating) return
        
        // 목표 상태가 현재 상태와 동일하면 처리 불필요
        ...
        // 펼칠 때 짧은 지연(50ms), 접을 때 긴 지연(200ms)으로 안정성 증가
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // 대기 중인 상태가 여전히 실행 필요한지 확인 (이후 스크롤에 의해 덮어쓰였을 수 있음)
    ...
    },
    {
      ...
      // rootMargin 조정, 뷰포트 하단을 위로 축소하여 더 많이 스크롤해야 펼침 트리거
...
// keep-alive 사용 시, 컴포넌트 활성화 시 데이터 재로드
onActivated(() => {
  loadHistory()
})
...
.grid-pattern {
  ...
  /* 왼쪽 상단에서 시작, 높이 변경 시 하단으로만 확장하여 기존 격자 위치에 영향 없음 */
...
/* 진행 상태 색상 */
.card-progress.completed { color: #10B981; }    /* 완료됨 - 초록색 */
.card-progress.in-progress { color: #F59E0B; }  /* 진행 중 - 주황색 */
.card-progress.not-started { color: #9CA3AF; }  /* 시작 안 됨 - 회색 */
